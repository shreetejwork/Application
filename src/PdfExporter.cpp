                                      ...
                                      const QString      &filePath)
{
    ...
    // ── DUAL TABLE GEOMETRY ──────────────────────────────────────────────────
    // The content area is split into two equal table sections with a gap between
    const int tableSectionGap = 14;                              // gap between left and right table sections
    const int sectionW        = (contentW - tableSectionGap) / 2; // width of each table section

    const int nCols = 4;
    // Column widths for each half-table (must sum to sectionW)
    int colW[nCols];
    colW[0] = 36;                                  // S/No.
    colW[1] = 68;                                  // Date
    colW[2] = 62;                                  // Time
    colW[3] = sectionW - colW[0] - colW[1] - colW[2]; // Reject Count / Batch Status
    ...
    for (const QVariant &v : rejectionData) {
        ...
        // Only count actual rejection rows, not status event rows
    ...
    // xOffset = left edge of this table section
    ...
    // Returns the display string and color for a row's last column.
    // eventType: "reject" | "start" | "pause" | "resume" | "end" | ""
    struct RowDisplay {
        QString text;
        QColor  color;
        bool    bold;
    ...
    auto resolveRowDisplay = [&](const QVariantMap &m) -> RowDisplay {
        ...
        if (eventType == "start")
            return { "Batch Start",   QColor(20, 130, 20),   true };
        if (eventType == "pause")
            return { "Batch Paused",  QColor(200, 80, 0),    true };
        if (eventType == "resume")
            return { "Batch Resumed", QColor(0, 80, 180),    true };
        if (eventType == "end")
            return { "Batch End",     QColor(60, 60, 60),    true };

        // Default: normal reject count row
    ...
    auto drawDataRow = [&](int xOffset, int y, int idx, const QVariantMap &m) {
        ...
        int x = xOffset;
        for (int c = 0; c < nCols; ++c) {
            // Separator line under each row (light)
            setPen(1, QColor(210, 210, 210));
            painter.drawLine(xOffset + 8, y + rowH - 1, xOffset + sectionW - 8, y + rowH - 1);

            Qt::Alignment al = (c == 0)
                                   ? (Qt::AlignVCenter | Qt::AlignHCenter)
                                   : (Qt::AlignVCenter | Qt::AlignLeft);

            // Last column gets status color + optional bold
            if (c == nCols - 1) {
    ...
    // Draws headers for both left and right table sections, returns Y after headers
    ...
    // Estimate fixed content height on page 0
    ...
    // On page 0 the two tables run side by side so rows consumed = pairs
    // Each "pair" of rows fills one row height in both sections simultaneously
    const int rowPairsPage0 = qMax(0, availPage0 / rowH);
    const int rowsPage0     = rowPairsPage0 * 2; // total rows consumed across both sections
    ...
    if (!rejectionData.isEmpty()) {
        ...
        int tableTopY = y; // remember where rows started for divider

        // Fill left section first, then right section simultaneously row by row
        // We track left and right indices separately
        // Left section: rows dataIndex .. dataIndex + rowPairsPage0 - 1
        // Right section: rows dataIndex + rowPairsPage0 .. dataIndex + rowsPage0 - 1
        // But we draw them paired (same Y) so we walk pairs

        int leftIdx  = dataIndex;
        int rightIdx = dataIndex + rowPairsPage0; // right section starts halfway
        int rowY     = y;
        ...
        // Draw vertical dashed divider between the two sections
        if (drewAnyRow)
            drawSectionDivider(tableTopY - 4, rowY);

        // Advance dataIndex past all rows drawn on this page
        // rightIdx tells us how far we got in the right section
        // The last consumed index overall is max(leftIdx, rightIdx) - 1
        dataIndex = rightIdx; // rightIdx is always >= leftIdx after the loop

        // If right section didn't reach rightIdx start (very few rows), clamp
        if (dataIndex > rejectionData.size())
            dataIndex = rejectionData.size();

        // Draw total row below right section if all data fits on page 0
    ...
    while (dataIndex < rejectionData.size()) {
        ...
        // "Rejection Details (continued)" label
        painter.setFont(fontB(10)); setPen(1);
        ...
        // How many rows fit per section on this page
        int rowsAvail     = 0;
        {
            int testY = y;
            while (testY + rowH <= footerDivY - 6) { testY += rowH; rowsAvail++; }
        }
        // Left section gets first half, right section gets second half
        int leftStart  = dataIndex;
        int rightStart = dataIndex + rowsAvail;
        ...
        // Advance dataIndex: everything up to rightIdx is consumed
        dataIndex = rightIdx;
        if (dataIndex > rejectionData.size())
            dataIndex = rejectionData.size();

        // Draw total row if this is the last page
        if (dataIndex >= rejectionData.size() && rowY + rowH <= footerDivY - 6)
            drawTotalRow(rightX, rowY);
