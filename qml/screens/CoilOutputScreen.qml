...
Item {
    ...
    function jumpToDay(day) {
        ...
        flickArea.contentX = Math.max(0, Math.min(targetX, flickArea.contentWidth - flickArea.width))
    }

    property int readingsPerDay: (24 * 60) / 5   // 288
    ...
    Component.onCompleted: {
        ...
        for (var day = 1; day <= totalDays; day++) {
            for (var r = 0; r < readingsPerDay; r++) {
                ...
                if (v < mn) mn = v
                if (v > mx) mx = v
                histValues.append({ dd: dd, mon: 1, hhmm: hhS + ":" + mmS, val: v })
            }
        }
    ...
    Rectangle {
        ...
        // ── bar geometry ──────────────────────────────────────────────────────
        property real barWidth:   Math.round(Math.max(1,   44 * root.scale * zoomFactor))
        property real barSpacing: Math.round(Math.max(0.5,  6 * root.scale * zoomFactor))
        ...
        // ── fixed layout constants (NO animated values here) ──────────────────
        ...
        // visible day (which day is at left edge of viewport)
        ...
        PinchHandler {
            target: null
            onActiveChanged: { if (active) histogramCard._pinchZoom = histogramCard.zoomFactor }
            onActiveScaleChanged: {
                var oldZ = histogramCard.zoomFactor
                var newZ = Math.max(histogramCard.minZoomFactor, Math.min(2.0, histogramCard._pinchZoom * activeScale))
        ...
        Item {
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    radius: 18 * root.scale; color: "#F7F9FD"
                    border.width: 1; border.color: "#DCE5F5"

                    RowLayout {
                        anchors.fill: parent
                        anchors.leftMargin: 18 * root.scale; anchors.rightMargin: 18 * root.scale
                        spacing: 14 * root.scale

                        ColumnLayout {
                            Layout.fillWidth: true; spacing: 3 * root.scale
                ...
                Rectangle {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            Layout.preferredWidth: 46 * root.scale; Layout.preferredHeight: 46 * root.scale
                            radius: 14 * root.scale
                            color: zoomOutMa.pressed ? "#CFE1FF" : (zoomOutMa.containsMouse ? "#E3EEFF" : "#FFFFFF")
                            ...
                            MouseArea {
                                id: zoomOutMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                            ...
                            Timer { id: zoomOutTimer; interval: 120; repeat: true
                                onTriggered: { if (histogramCard.zoomFactor <= histogramCard.minZoomFactor + 0.001) stop(); else zoomOutMa.doZoomOut() }
                            }
                        }

                        Rectangle {
                            Layout.fillWidth: true; Layout.preferredHeight: 46 * root.scale
                            radius: 14 * root.scale; color: "#FFFFFF"; border.width: 1; border.color: "#DCE5F5"
                            Column {
                        ...
                        Rectangle {
                            Layout.preferredWidth: 46 * root.scale; Layout.preferredHeight: 46 * root.scale
                            radius: 14 * root.scale
                            color: zoomInMa.pressed ? "#CFE1FF" : (zoomInMa.containsMouse ? "#E3EEFF" : "#FFFFFF")
                            border.width: 1; border.color: "#6F95D6"
                            ...
                            MouseArea {
                                id: zoomInMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                                function zoomInStep() {
                                    var oldZ = histogramCard.zoomFactor
                                ...
                                Timer { id: zoomInTimer; interval: 90; repeat: true; running: false; onTriggered: zoomInMa.zoomInStep() }
                            }
                        }
                    }
                }
        ...
        Rectangle {
            ...
            width: histogramCard.width; height: 1; color: "#E1E8F5"
        }
        ...
        Item {
            ...
            Rectangle {
                ...
                border.color: "#6F95D6"; border.width: 1
                ...
                MouseArea {
                    ...
                    property bool holdActive: false
                    onClicked: { if (!holdActive) { var c = histogramCard.visibleDay; if (c > 1) root.jumpToDay(c-1) } }
                ...
                Timer { id: prevHoldTimer; interval: 120; repeat: true
                    onTriggered: { var c = histogramCard.visibleDay; if (c > 1) root.jumpToDay(c-1); else stop() }
                }
            }
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    property bool holdActive: false
                    onClicked: { if (!holdActive) { var c = histogramCard.visibleDay; if (c < root.totalDays) root.jumpToDay(c+1) } }
                ...
                Timer { id: nextHoldTimer; interval: 120; repeat: true
                    onTriggered: { var c = histogramCard.visibleDay; if (c < root.totalDays) root.jumpToDay(c+1); else stop() }
                }
            }
            ...
            Flickable {
                ...
                height: 40 * root.scale; clip: true
                ...
                Item {
                    ...
                    Rectangle {
                        id: selHighlight
                        width: 44 * root.scale; height: 34 * root.scale; radius: 9 * root.scale
                        y: (pillBg.height - height) / 2
                        x: (histogramCard.visibleDay - 1) * ((44 * root.scale) + (7 * root.scale))
                        color: "#1A4DB5"; border.color: "#6F95D6"; border.width: 1; z: 0
                        Behavior on x { NumberAnimation { duration: 180; easing.type: Easing.OutCubic } }
                    }

                    Row {
                        id: pillRow
                        spacing: 7 * root.scale; anchors.verticalCenter: parent.verticalCenter
                        Repeater {
                ...
                Connections {
                    ...
                    function onVisibleDayChanged() {
                        var iw = (44 * root.scale) + (7 * root.scale)
                        var tx = (histogramCard.visibleDay - 1) * iw - dayPillFlick.width/2 + iw/2
                        dayPillFlick.contentX = Math.max(0, Math.min(tx, dayPillFlick.contentWidth - dayPillFlick.width))
                    }
                }
            }
        }
        ...
        Rectangle {
            ...
            width: histogramCard.width; height: histogramCard.needsNav ? 1 : 0
            color: "#E1E8F5"
        }
        ...
        property real graphTop: {
            ...
            t += 10 * root.scale   // small spacer
            return t
        }

        // graphH = remaining height inside card
        property real graphH: height - graphTop - padV

        // plot column width = card inner width minus y-axis strip
        property real plotW:  width - padH * 2 - yAxisW

        // barAreaH = graphH minus x-axis row minus top-pad for value labels
        property real barAreaH: Math.max(80 * root.scale, graphH - xAxisH - topPad)

        property real totalBarsW: Math.max(plotW, barCount * barStride + barSpacing * 2)
        ...
        Item {
            ...
            // Vertical axis line (right edge)
            Rectangle {
                anchors.right: parent.right
                anchors.top: parent.top; anchors.bottom: parent.bottom
                width: 2; color: "#4A5E8A"
            }
        ...
        Flickable {
            ...
            WheelHandler {
                ...
                onWheel: function(ev) {
                    var oldZ = histogramCard.zoomFactor
                    var f    = ev.angleDelta.y > 0 ? 1.08 : 0.92
                    var newZ = Math.max(histogramCard.minZoomFactor, Math.min(2.0, oldZ * f))
                    if (Math.abs(newZ - oldZ) < 0.005) return
                    var mcx = flickArea.contentX + ev.x
                    histogramCard.zoomFactor = newZ
                    flickArea.contentX = Math.max(0, mcx * (newZ / oldZ) - ev.x)
                }
            }
            ...
            Item {
                ...
                Repeater {
                    model: histogramCard.ySteps + 1
                    delegate: Rectangle {
                        x: 0; y: (index / histogramCard.ySteps) * plotItem.height
                        width: plotItem.width
                        height: index === histogramCard.ySteps ? 2 : 1
                        color:  index === histogramCard.ySteps ? "#4A5E8A" : "#E1E8F5"
                    }
                }
                ...
                Rectangle {
                    x: 0
                    y: (1 - (root.statAvg - histogramCard.yMin) /
                            (histogramCard.yMax - histogramCard.yMin)) * plotItem.height
                    width: plotItem.width; height: 1
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        Rectangle {
                            id: bar
                            property real fraction: (model.val - histogramCard.yMin) /
                            ...
                            antialiasing: false
                            gradient: Gradient {
                                GradientStop { position: 0.0; color: barSlot.isSelected ? "#FFB74D" : "#58A6FF" }
                                GradientStop { position: 1.0; color: barSlot.isSelected ? "#F57C00" : "#1A4DB5" }
                            }
                        }

                        // value label above bar (detail mode only)
                        Text {
                            visible: histogramCard.isDetail && histogramCard.zoomFactor >= 0.8
                            anchors.bottom: bar.top; anchors.bottomMargin: 4 * root.scale
                            anchors.horizontalCenter: bar.horizontalCenter
                            text: model.val.toLocaleString()
                            font.pixelSize: Math.max(8, 9 * root.scale); font.bold: true
                            color: "#1A4DB5"; z: 5
                            opacity: bar.height > 20 * root.scale ? 1 : 0
        ...
        Item {
            ...
            // scrolling label strip (mirrors flickArea.contentX)
            Item {
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        Rectangle {
                            ...
                            width: 1; height: 5 * root.scale; color: "#4A5E8A"
                        }

                        // Normal (zoomed-out) label — only at first reading of each day
                        Column {
                            visible: !histogramCard.isDetail && model.hhmm === "00:05"
                            anchors.top: parent.top; anchors.topMargin: 9 * root.scale
                            anchors.horizontalCenter: parent.horizontalCenter
                            spacing: 2 * root.scale
                            Text { anchors.horizontalCenter: parent.horizontalCenter; text: model.dd; font.pixelSize: Math.max(9,11*root.scale); font.bold: true; color: "#1A4DB5" }
                            Text { anchors.horizontalCenter: parent.horizontalCenter; text: root.monthName(model.mon); font.pixelSize: Math.max(8,9*root.scale); color: "#4A5E8A" }
                        }

                        // Detail label — every reading shows dd/mon/hhmm
                        Column {
                            ...
                            Text { anchors.horizontalCenter: parent.horizontalCenter; text: model.dd; font.pixelSize: Math.max(9,11*root.scale); font.bold: true; color: "#1A4DB5" }
                            Text { anchors.horizontalCenter: parent.horizontalCenter; text: root.monthName(model.mon); font.pixelSize: Math.max(8,9*root.scale); color: "#4A5E8A" }
                            Text { anchors.horizontalCenter: parent.horizontalCenter; text: model.hhmm; font.pixelSize: Math.max(7,8*root.scale); color: "#7B8FAD" }
                        }
                    }
                }
            }
        ...
        // ── CORNER COVER (hides y-axis / x-axis overlap) ─────────────────────
        Rectangle {
            x: histogramCard.padH
            y: histogramCard.graphTop + histogramCard.topPad + histogramCard.barAreaH
            width:  histogramCard.yAxisW
        ...
        Rectangle {
            id: tooltip
            visible: histogramCard.selectedIndex >= 0 && histogramCard.isDetail
            z: 200

            property real curVal: histogramCard.selectedIndex >= 0
                                  ? histValues.get(histogramCard.selectedIndex).val : 0
            property real pct: (curVal - histogramCard.yMin) /
                               (histogramCard.yMax - histogramCard.yMin) * 100

            x: Math.min(Math.max(8*root.scale, histogramCard.selectedBarX - width/2),
                        histogramCard.width - width - 8*root.scale)
            y: histogramCard.selectedBarY - height - 10 * root.scale

            width:  Math.max(110*root.scale, tipCol.implicitWidth + 24*root.scale)
            height: tipCol.implicitHeight + 18*root.scale
            radius: 10*root.scale; color: "#102B63"; border.width:1; border.color:"#5D86D8"
            opacity: visible ? 1 : 0; scale: visible ? 1.0 : 0.9
            Behavior on opacity { NumberAnimation { duration: 120 } }
            Behavior on scale   { NumberAnimation { duration: 120; easing.type: Easing.OutBack } }

            Column {
                id: tipCol; anchors.centerIn: parent; spacing: 2*root.scale
                Text { anchors.horizontalCenter: parent.horizontalCenter; text: Number(tooltip.curVal).toLocaleString(); font.pixelSize: 17*root.scale; font.bold: true; color: "#FFFFFF" }
                Text { anchors.horizontalCenter: parent.horizontalCenter; text: tooltip.pct.toFixed(1)+"%"; font.pixelSize: 11*root.scale; font.bold: true; color: "#AFCBFF" }
                Text {
                    anchors.horizontalCenter: parent.horizontalCenter
                    text: histValues.get(histogramCard.selectedIndex).dd + " " +
                          root.monthName(histValues.get(histogramCard.selectedIndex).mon) + "  " +
                          histValues.get(histogramCard.selectedIndex).hhmm
                    font.pixelSize: 10*root.scale; color: "#D6E4FF"
                }
            }
            Canvas {
                width: 14*root.scale; height: 8*root.scale
                ...
                onPaint: {
                    ...
                    c.fillStyle = "#102B63"; c.beginPath()
                    c.moveTo(0,0); c.lineTo(width/2,height); c.lineTo(width,0); c.closePath(); c.fill()
                }
            }
        }
    }
